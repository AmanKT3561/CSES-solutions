...
public:
    SparseTable( vector<int>& arr) {
        ...
        sparselogarray[1] = 0;
        for (int i = 2; i <= n; i++) sparselogarray[i] = sparselogarray[i/2] + 1;
...
signed main()
{
    ...
    function<long long(int,bool,bool,int,string&)> func = [&](int idx , bool tight , bool lz , int prev,string &res){
     if(idx == res.size()) return 1LL;
     if(dp[idx][tight][lz][prev] != -1) return dp[idx][tight][lz][prev];
     int ub = (tight == 1)? (res[idx] - '0'):9;
     long long ans = 0;
     for(int i = 0 ; i<=ub ; i++){
        if(i == prev && !lz) continue;
        ans += func(idx+1 , (tight && i == res[idx] - '0') , (lz && i == 0) , i , res);
     }
     dp[idx][tight][lz][prev] = ans;
    return ans;
    ...
    long long x = func(0 , 1 , 1 , 10 , s);
    memset(dp , -1 , sizeof(dp));
    long long y = func(0 , 1, 1 , 10 , t);
    long long anss = y-x;
